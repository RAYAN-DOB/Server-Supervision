...
export default function SitesPage() {
  const { sites, setSites, filters, setFilters } = useStore();
  ...
  const filteredSites = sites.filter((site) => {
    ...
    if (
      ...
      !filters.siteType.includes(site.type)
    )
      return false;
    if (
      filters.status.length > 0 &&
      !filters.status.includes(site.status)
    )
      return false;
    return true;
  });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-500 font-light">
            ...
            {filteredSites.length > 1 ? "s" : ""} sur {sites.length}
          </p>
        </motion.div>
          ...
          {showFilters && (
            ...
            >
              <div className="flex flex-wrap gap-2">
                <span className="text-xs text-gray-500 font-light mr-2">
                  Type :
                </span>
                {siteTypes.map((type) => (
                  <button
                    ...
                    onClick={() => {
                      if (filters.siteType.includes(type)) {
                        setFilters({
                          siteType: filters.siteType.filter((t) => t !== type),
                        });
                      } else {
                        setFilters({
                          siteType: [...filters.siteType, type],
                        });
                      }
                    }}
                    className={cn(
              ...
              >
                <div className="clean-card p-6 h-full cursor-pointer group">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <Badge
                      variant={
                        site.status as "ok" | "warning" | "critical" | "default"
                      }
